...
/*
    ...
    <this will allow me to distinguish the deleted files or new files, so the
   file transfer works properly>

*/

// find the root dir of the project
// project_root_path passed as reference so it can be altered to the actual
// wanted path
int find_project_index(std::string &project_root_path) {
  ...
  while (true) {
    ...
    if (fs::exists(index_path_tmp)) {
      std::cout << "Found root project under: " << project_root_path
                << std::endl;
      return 0;
    }
    ...
    if (project_root_path == previous_path || project_root_path.empty()) {
      std::cerr << "Aborting: Project doesn't exist. Create one first."
                << std::endl;
      return -1;
    }
  }
...
void load_stage_lines(std::string stage_path,
...
void change_stage_file_line(std::string to_change, std::string type,
                            std::string file_hash, std::string add_rm,
                            std::vector<std::string> &stage_lines) {

  int line_num = 0;
  // "line_to_insert": holds where we should add the new line in the stage file
  // if a file of the same sub-directories is found in case it exists already,
  // it simply changes the line if both cases above are not verified, simply add
  // to the end
  ...
  std::string new_line =
      to_change + "|" + type + "|" + file_hash + "|" + add_rm;
  ...
  // if file still doesnt exist and no similar subdirectorie is found, add to
  // the end
  if (line_to_insert == -1) {
...
void add_file(const std::string &project_root_path, std::string hash,
              const std::string &file_path, ProjectIndex &old_index,
              std::string add_rm, std::string type,
              std::vector<std::string> &stage_lines) {
  ...
  std::string to_change =
      fs::relative(absolute_path, project_root_path).string();

  bool exists = fs::exists(absolute_path);
  if (!exists &&
      add_rm == "1") { // if it was "0", it was supposed to be missing
    std::cout << "WARNING: File in " << file_path << "doesnt exists"
              << std::endl;
    return;
  }

  bool is_dir = fs::is_directory(absolute_path);
  // hash is only empty when the function is called by the main add, and not the
  // add_dot same for type, but no need for 2 verifications
...
int add_dot(const std::string &project_root_path,
            const std::string &where_called, ProjectIndex &old_index,
            std::vector<std::string> &stage_lines) {

  ProjectIndex change_index;

  // creating the index of the path where_called
  for (fs::recursive_directory_iterator it(where_called), end; it != end;
       ++it) {
    ...
    // TODO: 'gitignore'
    if (filename == ".git" || filename == "target") {
    ...
    auto old_it = old_index.find(
        fs::relative(dirEntry.path(), project_root_path).string());
    if (old_it != old_index.end()) {
      old_it->second.visited = true;
    }
  ...
  std::string where_called_relative =
      fs::relative(where_called, project_root_path).string();
  ...
  for (auto it = old_index.begin(); it != old_index.end();) {
    ...
    if (!it->second.visited && inside_dir(path, where_called_relative)) {
      ...
      add_file(project_root_path, it->second.hash, path, old_index, "0", type,
               stage_lines);
...
void write_back_stage(const std::string stage_path,
                      std::vector<std::string> stage_lines) {
...
int add(std::vector<std::string> files_to_commit) {
  ...
  for (std::string file : files_to_commit) {
    if (file == ".") {
      ...
      // allows user to input path to file starting with slash("/") or without
      // it
      std::string slash = "";
      if (!(file.front() == '/'))
        slash = "/";
      ...
      if (fs::exists(file) &&
          inside_dir(file,
                     project_root_path)) // in case the file is an absolute path
        file_path = file;
      // add necessary changes to both index and stage related to the file
      add_file(project_root_path, "", file_path, old_index, "1", "",
               stage_lines);
    }
  }
