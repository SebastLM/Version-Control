...
/*
  this function is needed because uint64_t needs a 64-bit conversion for network
  to host translation
*/
...
// the purpose of this function is to keep on receving the commited files from a
// user
int recv_file(int sock, bool is_dir) {
  ...
  // add the null character to the end of the string where we will store the
  // file this is the constructor of the std::string class
  ...
  // safeguard for file directories
  std::error_code er;
  fs::path fpath(name);
  if (fpath.has_parent_path()) {
    fs::create_directories(fpath.parent_path(), er);
    if (er) {
  ...
  int i = 1;
  while (file_size > 0) {
    // we do this to deal with the buffer not filling, and so we know the actual
    // recived len
  ...
  if (!hashing(file_in, hash_buffer, len_hash))
    throw std::runtime_error("error calculating hash value");
  if (len_hash != 32)
  ...
  /*
    ...
    // for this i will not close the out stream earlie, Will instead
    flush(file.flush()) ->
    // then in case of hash being different will need to move the pointer to the
    beggining for writing(so im not writing at the end but instead replacing the
    corrupted file)
  */
  std::cout << "\n\n";
